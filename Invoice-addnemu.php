       ...
       if ($count > 0){
        for ($i = 0; $i < $count; $i++){
          if (trim($_POST['item_invoice'][$i]) !=''){
           $sql2 = "INSERT INTO `add1`( `projact_invoice`,`item_invoice`,`desciption_invoice`,`quantity_invoice`,`price_invoice`)
           VALUES('$projact_invoice','$item_invoice[$i]','$desciption_invoice[$i]','$quantity_invoice[$i]','$price_invoice[$i]')";
     ...
     mysqli_close($con);//ปิดการเชื่อมต่อ database เพื่อคืนค่าให้กับ server
       ...
       if ( $result2){
        echo "<script type='text/javascript'>";
            echo "window.location='kuntest.php?projact_invoice=$projact_invoice'";
        echo "</script>";
        }else{
            echo "<script type='text/javascript'>";
                echo "alert('Error');";
